...
The server holds desired state per machine. A `machines` table gives every machine its own token bound to one user, and a `machine_skills` table records which skills that machine should have. The local disk holds actual state. Sync reconciles the two, installing what is missing and updating what is stale, and it asks the user before touching anything locally modified or locally deleted.
